...
//
//   - the exact bytes `DTSTART:20270206T060000Z` for the real Ceremony_Datetime,
...
describe('toIcs — the Ceremony_Datetime in UTC (9.3)', () => {
  ...
  it('serialises the Ceremony_Datetime as DTSTART:20270206T060000Z', () => {
    // Not `toContain` on a substring of a timestamp: the whole line, so a stray
    // trailing character or a missing `Z` fails. 14:00 +08:00 is 06:00 UTC.
    expect(ics.split(CRLF)).toContain('DTSTART:20270206T060000Z')
  })

  it('serialises the 90-minute duration as DTEND:20270206T073000Z', () => {
    // 06:00 UTC + 90 min = 07:30 UTC.
    expect(weddingConfig.calendar.durationMinutes).toBe(90)
    expect(ics.split(CRLF)).toContain('DTEND:20270206T073000Z')
  })
